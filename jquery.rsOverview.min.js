...
(function ($) {
    var OverviewClass = function (element, opts, scrollbarPixels) {
        ...
        }, onRender = function (event) { var calcWidth = (viewport.sizeX - scrollbarPixels) / coef, calcHeight = (viewport.sizeY - scrollbarPixels) / coef; viewportDiv.width(calcWidth).height(calcHeight).css('left', (viewport.obj.scrollLeft() / coef) + (opts.center ? contentDiv.position().left : 0)).css('top', (viewport.obj.scrollTop() / coef) + (opts.center ? contentDiv.position().top : 0)); }, init = function () {
    ...
    $.fn.rsOverview = function (options) { var opts = $.extend({}, $.fn.rsOverview.defaults, options), scrollbarWidth = function () { var calcDiv = $("<div style='visibily: hidden; overflow: scroll; width: 100px;'></div>"); $("body").append(calcDiv); try { return 100 - calcDiv[0].clientWidth; } finally { calcDiv.remove(); } }, scrollbarPixels = opts.viewport === window ? 0 : scrollbarWidth(); return this.each(function () { new OverviewClass($(this), opts, scrollbarPixels); }); }; $.fn.rsOverview.contentSizeChanged = function (element) { element.trigger('rsOverview.resize'); }; $.fn.rsOverview.defaults = { viewport: window, center: true, autoHide: false, scrollSpeed: 'medium' };
})(jQuery);